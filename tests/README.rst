...
fixture. We also define a ``dexcom_authorized`` fixture,
which will use a ``MemoryStorage`` object to tell Flask-Dance
that the user is already authorized with Dexcom.
...
When making live HTTP requests to Dexcom, such as when you record
new HTTP sessions, you must have a valid OAuth token for Dexcom.
Put this OAuth token in the ``DEXCOM_OAUTH_ACCESS_TOKEN`` environment
